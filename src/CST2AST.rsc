...
AQuestion cst2ast(qe:Question q) {
  switch (q) {
  	case (Question)`<Str label> <Id variable>:<Type t>`: return single("<label>", "<variable>", cst2ast(t), src=qe@\loc);
	case (Question)`<Str label> <Id variable>: <Type t> = <Expr e>`: return computed("<label>", "<variable>", cst2ast(t), cst2ast(e), src=qe@\loc);
	case (Question)`{<Question* qs>}`: return block([cst2ast(q) | Question q <- qs], src=qe@\loc); 
	case (Question)`if (<Expr e>) {<Question* qs>}`: return ifthen(cst2ast(e), [cst2ast(q) | Question q <- qs], src=qe@\loc);
	case (Question)`if (<Expr e>) {<Question* qs1>} else {<Question* qs2>}`: return ifthenelse(cst2ast(e), [cst2ast(q) | Question q <- qs1], [cst2ast(q) | Question q <- qs2], src=qe@\loc);
...
AExpr cst2ast(ex:Expr e) {
  switch (e) {
  	case (Expr)`(<Expr e>)`: return parentheses(cst2ast(e), src=ex@\loc);
    ...
    case (Expr)`<Str x>`: return string("<x>", src=ex@\loc);
